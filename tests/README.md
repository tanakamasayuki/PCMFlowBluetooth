...
- `peer/a2dp_sbc_receive/` — Real A2DP on two boards: connect, codec configuration, sustained SBC reception, PCM decode, suspend/resume, disconnect, reconnect.
...
The peer side of `peer/a2dp_sbc_receive/` sends a **pre-generated read-only SBC vector embedded in the test image** through `EspBleClassicA2dpSource::send()`, rather than encoding in real time. That is deterministic, so the DUT-side PCM can be checked exactly across resume and reconnect.
